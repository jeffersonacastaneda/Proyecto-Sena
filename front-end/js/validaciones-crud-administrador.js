import {validarCampos,limpiarCampos,validarISBN,validarBusqueda,llenarTabla,
} from "./validaciones-formularios.js";
...
document.getElementById("btn-agregar").addEventListener("click", function (event) {
    ...
    if (isValid && isIsbn) {
      fetch("/libros/agregar", {method: "POST",headers: { "Content-Type": "application/json" },
...
document.getElementById("btn-buscar").addEventListener("click", function (event) {
  ...
  // Evita el envío automático del formulario
    ...
    if (isTitle) {fetch(`/libros/buscar?titulo=${encodeURIComponent(titulo)}`, {
        method: "GET",
