...
document.getElementById("btn-prestar").addEventListener("click", function (event) {
...
document.getElementById("btn-devolver").addEventListener("click", function (event) {
  ...
  // Evita el envío automático del formulario
    ...
    fetch("/libros/devolverP", {method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify(requestData),
...
document.getElementById("btn-buscar-isbn").addEventListener("click", (event) => {
    event.preventDefault();
    const isbn = document.getElementById("isbn").value;
