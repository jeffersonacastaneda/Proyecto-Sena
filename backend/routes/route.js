...
const connection = require('../config/dbConfig'); // Importar la conexión desde db.js
...
router.get("/buscar", (req, res) => {const { titulo } = req.query;
  console.log(`Buscando libro con título: ${titulo}`); // Verifica el título recibido

  const query ="SELECT * FROM material_bibliografico WHERE LOWER(titulo) = LOWER(?)";
...
router.put("/modificar/:isbn", (req, res) => {const { isbn } = req.params; // Obtener el ISBN del parámetro de la URL
  const { titulo, autor, editorial, genero } = req.body;

  const query = `UPDATE material_bibliografico SET titulo = ?, autor = ?, editorial = ?, genero = ? WHERE isbn = ?`;
...
router.delete("/eliminar/:isbn", (req, res) => { const { isbn } = req.params;
...
// Ruta para buscar invitado

router.get("/buscar-invitado", (req, res) => {const { titulo, autor, genero } = req.query;
let query ="SELECT titulo, autor, genero FROM material_bibliografico WHERE 1=1";
  const params = [];
...
router.post("/agregarP", async (req, res) => {
  const {nombre,apellido,telefono,direccion,correo,isbn, fechaP,fechaD,} = req.body;
  ...
  try {
    ...
    const [bookResults] = await connection
      .promise()
      .execute("SELECT id, disponibilidad FROM material_bibliografico WHERE isbn = ?",[isbn]);
    ...
    await connection
      .promise()
      .execute("INSERT INTO prestamo (usuario_id, material_id, fecha_prestamo, fecha_devolucion) VALUES (?, ?, ?, ?)",[userId, materialId, fechaP, fechaD]);
    console.log("Préstamo registrado correctamente.");

    // Actualizar disponibilidad del libro a 0 (no disponible)
    await connection
      .promise()
      .execute("UPDATE material_bibliografico SET disponibilidad = 0 WHERE id = ?",[materialId]);
...
router.post("/devolverP", async (req, res) => {const { isbn } = req.body; // Recibimos el ISBN del libro

  try {console.log("Datos recibidos:", req.body); // Log de los datos recibidos
    ...
    await connection.promise()
      .execute("UPDATE material_bibliografico SET disponibilidad = 1 WHERE id = ?",
        [materialId]
      );
...
router.get("/buscar-isbn", (req, res) => {
  ...
  const query ="SELECT * FROM material_bibliografico WHERE isbn = ?";
